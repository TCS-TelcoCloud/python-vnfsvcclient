...
from vnfsvcclient.common import command
...
def find_resourceid_by_name_or_id(client, resource, name_or_id,
    ...
    except exceptions.VNFSvcClientException:
...
class VNFSvcCommandMeta(abc.ABCMeta):
    def __new__(cls, name, bases, cls_dict):
        if 'log' not in cls_dict:
            cls_dict['log'] = logging.getLogger(
                cls_dict['__module__'] + '.' + name)
        return super(VNFSvcCommandMeta, cls).__new__(cls,
                                                      name, bases, cls_dict)


@six.add_metaclass(VNFSvcCommandMeta)
class VNFSvcCommand(command.OpenStackCommand):
    ...
    def __init__(self, app, app_args):
        super(VNFSvcCommand, self).__init__(app, app_args)
        # NOTE(markmcclain): This is no longer supported in cliff version 1.5.2
        # see https://bugs.launchpad.net/python-vnfsvcclient/+bug/1265926
    ...
    def get_client(self):
        return self.app.client_manager.vnfsvc
...
class CreateCommand(VNFSvcCommand, show.ShowOne):
...
class UpdateCommand(VNFSvcCommand):
...
class DeleteCommand(VNFSvcCommand):
...
class ListCommand(VNFSvcCommand, lister.Lister):
    ...
    def call_server(self, vnfsvc_client, search_opts, parsed_args):
        resource_plural = _get_resource_plural(self.cmd_resource,
                                               vnfsvc_client)
        obj_lister = getattr(vnfsvc_client, "list_%s" % resource_plural)
        if self.parent_id:
    ...
    def retrieve_list(self, parsed_args):
        """Retrieve a list of resources from VNFSvc server"""
        vnfsvc_client = self.get_client()
        vnfsvc_client.format = parsed_args.request_format
        _extra_values = parse_args_to_dict(self.values_specs)
        _merge_args(self, parsed_args, _extra_values,
        ...
        data = self.call_server(vnfsvc_client, search_opts, parsed_args)
        collection = _get_resource_plural(self.resource, vnfsvc_client)
...
class ShowCommand(VNFSvcCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        obj_shower = getattr(vnfsvc_client, "show_%s" % self.cmd_resource)
        if self.parent_id:
